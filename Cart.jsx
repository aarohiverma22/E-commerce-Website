...
function Cart() {
  const dispatch = useDispatch(); //perform update action
  const result = useSelector((state) => state.product.cart);  //add product to cart

  function handleremove(id) {
    dispatch(removeitem(id)); //remove item based on id
  }

 
  const totalPrice = result.reduce((total, item) => total + item.price, 0);  //calculate total with initial 0 value
